...
def get_graphs(dsname, nodes_df, sample_mode, feat, partition, concat_all_absdf, load_features):
    ...
    node_len = []
    printed = 0
    #was_vuln = [] Eemaldasin vea parast koodis
    for graph_id, group in tqdm.tqdm(nodes_df.groupby("graph_id"), f"graphize {partition}"):
        ...
        #was_vuln.append(group["vuln"].max().item()) Eemaldasin vea parast koodis
    ...
    #logger.info("percentage of vuln graphs: %s", np.average(was_vuln)) Eemaldasin vea parast koodis
