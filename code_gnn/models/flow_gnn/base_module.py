...
class BaseModule(pl.LightningModule):
    def __init__(
        ...
        #self.test_pr_curve_bin = BinnedPrecisionRecallCurve(1) Eemaldasin BinnedPrecisionRecallCurve-i sest pytorch module ei suutnud seda importida               !!!!!
        self.test_preds = torchmetrics.CatMetric()
        self.test_labels = torchmetrics.CatMetric()
        self.test_confmat = torchmetrics.ConfusionMatrix(num_classes=2, task = "binary")
    ...
    def get_label(self, batch):
        if self.hparams.label_style == "node":
            ...
            print("Label content:\n")
            for g in graphs:
                print("Graph content : number of nodes - " + str(g.number_of_nodes()) + ", number of edges - " + str(g.num_edges()) + ", classification length - " + str(len(g.ndata["_VULN"])) + ", classification 0 content - " + str(torch.sum(g.ndata["_VULN"] == 0)) + "\n")
            print("\n")
    ...
    def validation_epoch_end(self, outputs):
        ...
        nni.report_intermediate_result(ld["val_BinaryF1Score"].cpu().item()) # Muutsin nime val_F1Score-st val_BinaryF1Score-ks                 !!!!!
