		void CWebServer::RType_HandleGraph(WebEmSession & session, const request& req, Json::Value &root)
		{
								...
								if (nMeterType == 0)
								{
									...
									else
									{
										...
										if ((ntime.tm_hour != 0) && (ntime.tm_min != 0))
										{
											...
											getNoon(atime, ltime, ntime.tm_year + 1900, ntime.tm_mon + 1, ntime.tm_mday - 1);  
									...
									if (actDateTimeHour != LastDateTime || ((method == 1) && (itt + 1 == result.end())))
									{
										if (bHaveFirstValue)
										{
											...
											case MTYPE_ENERGY_GENERATED:
												sprintf(szTmp, "%.3f", (TotalValue / divider)*1000.0f);	 
												break;
											case MTYPE_GAS:
			...
			else if (srange == "week")
			{
				if (sensor == "rain") {
					...
					time_t weekbefore;
					struct tm tm2;
					getNoon(weekbefore, tm2, tm1.tm_year + 1900, tm1.tm_mon + 1, tm1.tm_mday - 7);  
			...
			else if ((srange.substr(0, 1) == "2") && (srange.substr(10, 1) == "T") && (srange.substr(11, 1) == "2"))  
			{
