static void multi_set_termios(struct sb_uart_port *port, struct MP_TERMIOS *termios, struct MP_TERMIOS *old)
{
	...
	serial_outp(mtpt, UART_LCR, cval | UART_LCR_DLAB); 

	serial_outp(mtpt, UART_DLL, quot & 0xff);      
	serial_outp(mtpt, UART_DLM, quot >> 8);        

	serial_outp(mtpt, UART_LCR, cval);         
	mtpt->lcr = cval;                  
