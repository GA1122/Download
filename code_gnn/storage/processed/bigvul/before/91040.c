		void CWebServer::HandleCommand(const std::string &cparam, WebEmSession & session, const request& req, Json::Value &root)
		{
			...
			else if (cparam == "addscenedevice")
			{
				...
				std::string color = _tColor(request::findValue(&req, "color")).toJSONString();  
			...
			else if (cparam == "switchmodal")
			{
				...
				std::string action = request::findValue(&req, "action"); 
				std::string onlyonchange = request::findValue(&req, "ooc"); 
				if ((idx.empty()) || (switchcmd.empty()))
					return;
				...
				if (m_mainworker.SwitchModal(idx, switchcmd, action, onlyonchange, until) == true) 
				{
					root["status"] = "OK";
					root["title"] = "Modal";
				}
