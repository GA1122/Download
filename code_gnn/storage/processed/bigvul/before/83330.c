bool CSoundFile::ProcessEffects()
{
	...
	ROWINDEX nBreakRow = ROWINDEX_INVALID;		 
	ROWINDEX nPatLoopRow = ROWINDEX_INVALID;	 
	...
	for(CHANNELINDEX nChn = 0; nChn < GetNumChannels(); nChn++, pChn++)
	{
		...
		bool triggerNote = (m_PlayState.m_nTickCount == nStartTick);	 
		...
		case CMD_ARPEGGIO:
			...
			if((!pChn->nPeriod || !pChn->nNote)
				&& (pChn->pModInstrument == nullptr || !pChn->pModInstrument->HasValidMIDIChannel())	 
				&& !m_playBehaviour[kITArpeggio] && (GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT))) break;
			if (!param && (GetType() & (MOD_TYPE_XM | MOD_TYPE_MOD))) break;	 
		...
		case CMD_KEYOFF:
			if(m_playBehaviour[kFT2KeyOff])
			{
				if (m_PlayState.m_nTickCount == param)
				{
					...
					{
						if(param == 0 && (pChn->rowCommand.instr || pChn->rowCommand.volcmd != VOLCMD_NONE))  
						{
							pChn->dwFlags.set(CHN_NOTEFADE);
						}
						else
