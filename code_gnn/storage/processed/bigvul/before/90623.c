  ...
  ExceptionInfo *exception)
{
  ...
  for (i=0; i < 8; i++)
  {
    cin.image.channel[i].designator[0]=0;  
    offset+=WriteBlobByte(image,cin.image.channel[0].designator[0]);
    cin.image.channel[i].designator[1]=(unsigned char) (i > 3 ? 0 : i+1);  ;
