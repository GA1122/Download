 int sandbox(void* sandbox_arg) {
 	...
 	if (arg_netfilter && any_bridge_configured()) {  
	...
	set_caps();
 	...
 	if (cfg.protocol) {
 		...
 		seccomp_load(RUN_SECCOMP_PROTOCOL);	 
 		protocol_filter_save();	 
 	}
 	else {
...
// 
 	...
 	int status = monitor_application(app_pid);	 
 	flush_stdin();
