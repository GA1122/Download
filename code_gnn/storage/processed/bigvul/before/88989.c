  ...
  for (y=0; y <= (ssize_t) clahe_info->y; y++)
  {
    ...
    for (x=0; x <= (ssize_t) clahe_info->x; x++)
    {
      ...
      InterpolateCLAHE(clahe_info,
        tiles+(number_bins*(tile.y*clahe_info->x+tile.x)),      
        tiles+(number_bins*(tile.y*clahe_info->x+offset.x)),    
        tiles+(number_bins*(offset.y*clahe_info->x+tile.x)),    
        tiles+(number_bins*(offset.y*clahe_info->x+offset.x)),  
        &tile,lut,p);
      p+=tile.width;
    }
